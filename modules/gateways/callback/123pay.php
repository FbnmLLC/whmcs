...
if ($State == 'OK') {
    ...
    if ($result->status) {
        ...
        if ($verifiedAmount == $amount) {
            ...
            logTransaction($GATEWAY["name"], array_merge($_REQUEST, array('status' => $result->status, 'message' => $result->message, 'amount' => $result->amount)), "Successful");
        } else {
            logTransaction($GATEWAY["name"], array_merge($_REQUEST, array('status' => $result->status, 'message' => $result->message, 'amount' => $result->amount)), "Unsuccessful");
        }
    }
} else {
